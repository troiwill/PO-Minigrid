...
def can_enter_cell(cell: WorldObj | None) -> npt.NDArray[np.bool_]:
    ...
    """
    return True if cell is None or cell.can_overlap() else False  # type: ignore


@np_vectorize(signature="()->()")
def can_pickup_cell(cell: WorldObj | None) -> npt.NDArray[np.bool_]:
    """Determines if an agent can pickup a given cell.

    This function checks whether a cell is empty or contains an item the agent can pickup.
    ...
    Notes:
        ...
        - The signature "()->()" indicates that the function takes a scalar input and returns a scalar output.
    """
    return True if cell is not None and cell.can_pickup() else False  # type: ignore
...
def get_grid_cell(
    grid: Grid, position: np.ndarray
) -> npt.NDArray[np.object_]:  # WorldObj or None
    ...
    """
    return grid.get(position[0], position[1])  # type: ignore


def headings_are_valid(thetas: np.ndarray) -> npt.NDArray[np.bool_]:
    ...
    """
    return bool(np.all(0 <= thetas) and np.all(thetas < 4))  # type: ignore
